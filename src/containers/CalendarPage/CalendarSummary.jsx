...
const CalendarSummary = () => (
  <div>
    <Heading1>Today<span>10</span></Heading1>
    <div>
      {getUpcomingFive(EVENTS).map(({ id, start, end, type, pet: { name, breed } }) => (
        <Card key={id}>
          <Flex>
            <EventIcon>{getMetaData(type)}</EventIcon>
            <div>
              <Heading4>{name}<span>, {breed}</span></Heading4>
              <EventDesc>{type} at {moment(start).format('LT')} - {moment(end).format('LT')}</EventDesc>
            </div>
          </Flex>
